...
export default function PixelSnow({
  ...
  // Hemat daya di HP: layar sentuh/kecil → pixel ratio 1 (bukan 2x) &
  // density lebih jarang. Dihitung sekali saat mount.
  ...
  // Prop `paused` (via ref) — true = render loop berhenti total (hemat GPU).
  // Dipakai BackgroundLayer untuk meredam salju di zona yang "tenang".
  const pausedRef = useRef(false);
  const rendererRef = useRef(null);
  const materialRef = useRef(null);
  const resizeTimeoutRef = useRef(null);

  // Sinkronkan nilai prop ke ref tanpa bikin scene dibuat ulang.
  pausedRef.current = paused;

  // Memoize shader variant value
  const variantValue = useMemo(() => { return variant === 'round' ? 1.0 : variant === 'snowflake' ? 2.0 : 0.0; }, [variant]);

  // Memoize color conversion
  const colorVector = useMemo(() => { const threeColor = new Color(color); return new Vector3(threeColor.r, threeColor.g, threeColor.b); }, [color]);
  ...
  useEffect(() => {
    ...
    const material = new ShaderMaterial({
      ...
      uniforms: {
        ...
        uGamma: { value: gamma },
        uDensity: { value: lowPower ? Math.min(density, 0.12) : density },
    ...
    const animate = () => {
      ...
      // Only render if visible & not paused — render WebGL dilewati
      // sepenuhnya saat pause (frame loop tetap jalan, murah).
