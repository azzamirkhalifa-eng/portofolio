  ...
  return (
    <div className="fixed bottom-5 right-5 z-[90] flex items-center gap-2">
      <Link
        to="/admin/dashboard"
        data-edit-nav
        className="hidden items-center gap-1.5 rounded-md border border-hairline bg-background/90 px-3.5 py-2 text-xs text-muted backdrop-blur transition-colors hover:border-white/25 hover:text-foreground sm:inline-flex"
      >
        Dashboard ↗
      </Link>
      ...
      <button
        ...
        className={`inline-flex items-center gap-2 rounded-md px-4 py-2 text-sm font-medium shadow-lg shadow-black/30 transition-colors ${
          enabled
            ...
            : 'border border-hairline bg-background/90 text-foreground backdrop-blur hover:border-white/30'
        }`}
      >
        <svg
          aria-hidden
